"""
Cursor project session management.

Simplified session binding for Cursor Agent CLI — pane-log based communication
when running in tmux pane, with fallback to direct subprocess.
"""
...
def find_project_session_file(work_dir: Path, instance: Optional[str] = None) -> Optional[Path]:
    ...
    filename = session_filename_for_instance(".cursor-session", instance)
...
def load_project_session(work_dir: Path, instance: Optional[str] = None) -> Optional[CursorProjectSession]:
...
def compute_session_key(session: CursorProjectSession, instance: Optional[str] = None) -> str:
